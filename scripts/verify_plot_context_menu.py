"""Phase 9 real desktop menu actions with isolated uploaded data and native 3.13 servers.
Global Python runs only stdlib/Playwright. Reuses existing server/fixture helpers.
"""
import argparse
import csv
import io
...
def run_checks(web, api, dataset, temporary, output):
    ...
    extension.mkdir()
    (extension / 'manifest.json').write_text(json.dumps({'manifest_version': 3, 'name': 'Plot menu zoom checks',
    ...
    with sync_playwright() as p:
        ...
        def action(target, name, method='right'):
            open_menu(target, method).get_by_role('menuitem', name=name, exact=True).click()
        ...
        def export(target, label):
            ...
            expect(target.get_by_role('button', name=re.compile('^Export .+ plot$'))).to_be_focused()
        ...
        try:
            ...
            original = target.get_by_role('button', name=re.compile('Show original'))
            open_menu(target).get_by_role('menuitemcheckbox', name='Show original with filtered', exact=True).click()
            expect(original).to_have_attribute('aria-pressed', 'false')
            original.click(); settle()
            ...
            action(target, 'Reset Y axis (this plot)'); settle()
            ...
            for name, key in [('Full test', 'full'), ('Spectrum', 'spectrum'), ('XY', 'xy')]:
                ...
                export(target, key)
            print('PASS: eight native CSV/PNG metadata downloads across Time/Full/Spectrum/XY, exact API replay, failure/Close, shared resets', flush=True)
...
def main():
    ...
    parser.add_argument('--frontend-port', type=int, default=3270)
    parser.add_argument('--backend-port', type=int, default=8270)
    args = parser.parse_args()
    output = ROOT / 'data/verification/plot-context-menu'; output.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='ptt-menu-') as directory:
