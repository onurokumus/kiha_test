...
TP original traces keep ordered minimum/maximum samples at their actual times (plus endpoints); filtered envelopes use local buckets whose timestamps are their first samples. They share interval and origin, but can have different time arrays, bucket widths and retained points. They must be separate time facets, not zipped together or used to compute pointwise differences from reduced arrays. Filtered TP computation uses the complete TP regardless of subsequent client-side plot zoom.
